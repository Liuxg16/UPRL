...
#from bert_sim.score1 import Bert_sim
...
class RNNModel(nn.Module):
	...
	def forward(self, input, target, predictflag = True):
		'''
		bs,15; bs,15
		'''
...
class UPRL_LM1(nn.Module):
    def __init__(self, option):
        ...
        self.mlp1 = nn.Linear(ninp+2*self.topk+2,100)
        self.nhid = nhid
        self.device = torch.device("cuda" if torch.cuda.is_available() and not self.option.no_cuda else "cpu")
    ...
    def forward(self, input, key_pos, sequence_length, forwardmodel, backwardmodel,embmodel, id2sen=None):
        ...
        self.switch_m = torch.FloatTensor(self.batch_size, 2).fill_(self.prate).to(self.device)
        self.switch_m[:,1] = 1- self.prate
        self.action_m = torch.FloatTensor(self.batch_size, 2*self.topk+2).fill_(1.0/(4*self.topk+2)).to(self.device)

        for i in range(N_step): 
            pos = i% (self.length-2)+1
            #pos = i% (self.length-3)+2
            st,pi, action, length_t , re= self.step(st,s0, emb0, key_pos, pos, length_t,c0,h0,\
                    forwardmodel, backwardmodel, id2sen)
            pis[:,i:i+1] = pi
            actions[:,i:i+1] = action
            #print('-----pos', pos)
            #print(id2sen(st.cpu().numpy()[0]))
            #print(id2sen(re.cpu().numpy()[0]))
        step_reward = torch.sum(torch.eq(actions, 2*self.topk+1).float(),1) * self.step_reward # hold
        fv0, sim, div, flu0  = self.f(s0,s0, key_pos, forwardmodel, backwardmodel, embmodel, sequence_length, sequence_length,
                origin=True)
        fvt, sim, div, flu1  = self.f(st,s0, key_pos, forwardmodel, backwardmodel, embmodel, length_t, sequence_length)
        improve_flag = fvt/fv0
        reward = (1.0/10)*improve_flag + step_reward
        #reward = torch.sum(actions,1)*0.1
        reward = reward.detach() # bs,1
        reward =  reward.view(self.repeat_size, -1) 
        reward_adjust = nn.ReLU()(reward- torch.mean(reward,0, keepdim = True)) #rep,k
        ...
        sum_pi = torch.sum(-torch.log(torch.clamp(pis,1e-10,1-1e-10)),1, keepdim = True) # bs,1
        loss = sum_pi*(reward_adjust.view(-1,1)) # bs,1
        return loss, reward, st, flu0.detach(), flu1.detach()
    ...
    def step(self, s_t_1, s0, emb0, key_pos, pos, length_t,c0,h0, forwardmodel, backwardmodel,
        ...
        rep_words, insertwords, valid_id = self.predictwords(forwardmodel, backwardmodel, s_t_1,
                s0,pos,length_t, id2sen)

        K = 2*self.topk+2
        tag = torch.arange(K).view(-1,1).long()
        onehot = torch.zeros(K,K).scatter_(1,tag,int(1)).to(self.device)
        # bs,80
        rep_words = torch.cat([rep_words, insertwords], 1)
    ...
    def predictwords(self, forwardmodel ,backwardmodel, s_t_1, s0, pos, length_t, id2sen):
        K = self.topk-self.length
        output_f = forwardmodel(s_t_1, s_t_1, predictflag = True) # bs,length, v
        # for backward
        batch_size = s0.size(0)
        valid_id = torch.gt(length_t-1,pos)
        revse_id =valid_id.long()*(length_t-pos-2)
        ids = torch.zeros(batch_size, self.length).to(self.device)
        ones = torch.ones(batch_size,1).to(self.device)
        ids = ids.scatter(1, revse_id,ones)
        input = reverse_input(s_t_1.cpu(), length_t.cpu()).to(self.device)
        decoded = backwardmodel(input, input, predictflag = True)
        output_b = nn.Softmax(1)(decoded[ids>0.5])

        output_rep = nn.Softmax(1)(output_f[:,pos-1,:])*output_b
        output_rep[:,30000:] = 0
        prob, word = torch.topk(output_rep,K)
        word = torch.cat([word, s0],1)

        # for insertion
        revse_id =valid_id.long()*(length_t-pos-1)
        ids = torch.zeros(batch_size, self.length).to(self.device)
        ones = torch.ones(batch_size,1).to(self.device)
        ids = ids.scatter(1, revse_id,ones)
        output_b = nn.Softmax(1)(decoded[ids>0.5])
        output_ins = nn.Softmax(1)(output_f[:,pos-1,:])*output_b
        output_ins[:,30000:] = 0
        prob, ins_word = torch.topk(output_ins,K)

        #print('replace',id2sen(s_t_1.cpu().numpy()[0]))
        #print('replace',id2sen(ins_word.cpu().numpy()[0]))
        #assert False
    ...
    def f(self, st,s0, key_pos, forwardmodel=None, backwardmodel=None,  embmodel=None, sequence_length =None, original_length=None,
        ...
        expression_diversity = torch.clamp(1-(align/original_length.float().view(-1)),0.0,1)+0.2
        ...
        res = sim * expression_diversity *fluency 
        return res, sim, expression_diversity, fluency
...
class UPRL_LM(nn.Module):
    ...
    def forward(self, input, key_pos, sequence_length, forwardmodel, backwardmodel,embmodel, id2sen=None):
        ...
        fv0, sim, div, flu1  = self.f(s0,s0, key_pos, forwardmodel, backwardmodel, embmodel, sequence_length, sequence_length,
                origin=True)
        fvt, sim, div, flu2  = self.f(st,s0, key_pos, forwardmodel, backwardmodel, embmodel, length_t, sequence_length)
        improve_flag = fvt/fv0
        ...
        sum_pi = torch.sum(-torch.log(torch.clamp(pis,1e-10,1-1e-10)),1, keepdim = True) # bs,1
        loss = sum_pi*(reward_adjust.view(-1,1)) # bs,1
        return loss, reward, st, flu1.detach(), flu2.detach()
    ...
    def f(self, st,s0, key_pos, forwardmodel=None, backwardmodel=None,  embmodel=None, sequence_length =None, original_length=None,
        ...
        align_0 = align_0.unsqueeze(2).repeat(1,1,self.length)
        align = torch.sum(torch.sum(torch.eq(align_t,align_0).float(),1),1)
        expression_diversity = torch.clamp(1-(align/original_length.float().view(-1)),0.05,1)+0.2
        expression_diversity = expression_diversity**self.M_div
...
class UPRL_BERT(nn.Module):
    def __init__(self, option):
        super(UPRL_BERT, self).__init__()
        rnn_type = 'LSTM'
        self.option = option
        self.length = 15
        self.topk = option.topk
    ...
    def forward(self, input, key_pos, sequence_length, forwardmodel, backwardmodel,embmodel, id2sen=None):
        ...
        for i in range(N_step): 
            if N_step==self.length-2:
                pos = i% (self.length-2)+1
            elif N_step==self.length-3:
                pos = i% (self.length-3)+2
            st,pi, action, length_t , re= self.step(st,s0, emb0, key_pos, pos, length_t,c0,h0,\
                    forwardmodel, backwardmodel)
            pis[:,i:i+1] = pi
            ...
            #print(id2sen(re.cpu().numpy()[0]))
        step_reward = torch.sum(torch.eq(actions, 4*self.topk+1).float(),1) * self.step_reward # hold
        fv0, sim, div, flu1  = self.f(s0,s0, key_pos, forwardmodel, backwardmodel, embmodel,
                sequence_length, sequence_length, origin=True, id2sen=id2sen)
        fvt, sim, div, flu2  = self.f(st,s0, key_pos, forwardmodel, backwardmodel, embmodel,
                length_t, sequence_length, id2sen=id2sen)
        improve_flag = fvt/fv0
        reward = (1.0/10)*improve_flag* torch.gt(improve_flag,1).float() + step_reward
        #reward = torch.sum(actions,1)*0.1
        reward = reward.detach() # bs,1
    ...
    def f(self, st,s0, key_pos, forwardmodel=None, backwardmodel=None,  embmodel=None, sequence_length =None, original_length=None,
        ...
        s0s = [' '.join(id2sen(s, True)) for s in s0.cpu().tolist()]
        s1s = [' '.join(id2sen(s, True)) for s in st.cpu().tolist()]
        F = self.bertsim.cal_sim(s1s, s0s)
        bertsim = F.detach().to(self.device)**self.M_kw
        #bertsim = torch.tensor(F, dtype=sim.dtype, device=sim.device)**self.M_kw
        sim = sim*bertsim
...
class UPRL_T(nn.Module):
    def __init__(self, option):
        super(UPRL_T, self).__init__()
        self.option = option
        self.length = 15
        self.topk = option.topk
        self.num_edits = option.num_edits
        ...
        self.drop = nn.Dropout(dropout)
        # for transformer
        n_src_vocab = self.vocab_size
        len_max_seq = self.length
        d_word_vec = ninp
        d_model = self.hidden_size
        d_inner = self.hidden_size
        ...
        self.decoder = Decoder(
            n_tgt_vocab= n_src_vocab, len_max_seq=len_max_seq,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)

        self.mlp_del = nn.Linear(nhid+1+self.length, 1)
        self.mlp_hold = nn.Linear(nhid+1+self.length, 1)

        self.mlp_rep = nn.Linear(nhid+1+self.length, 100)
        self.mlp_ins = nn.Linear(nhid+1+self.length, 100)
        self.mlp1 = nn.Linear(ninp+2*self.topk,100)
    ...
    def forward(self, input, key_pos, sequence_length, forwardmodel, backwardmodel,embmodel, id2sen=None):
        ...
        seq_range =torch.arange(self.length).view(1,-1).long().to(self.device).repeat(self.batch_size,1) +1
        seq_pos = seq_range* (torch.le(seq_range, sequence_length.view(-1,1))).long()

        enc_output, *_ = self.source_encoder(input, seq_pos) # bs, l, d
        st = input.clone()
        length_t = sequence_length.clone()
        pis = torch.zeros(self.batch_size,N_step).to(self.device)
        actions = torch.zeros(self.batch_size,N_step).to(self.device)
        ...
        for i in range(N_step): 
            ...
            st,pi, action, length_t , re= self.step(st,s0, enc_output, key_pos, pos,\
                    length_t,seq_range,forwardmodel, backwardmodel, embmodel)
            pis[:,i:i+1] = pi
            actions[:,i:i+1] = action
            #print('-----pos', pos)
            #print(id2sen(st.cpu().numpy()[0]))
    ...
    def step(self, s_t_1, s0, enc_output, key_pos, pos, length_t, seq_range, forwardmodel,
            backwardmodel, embmodel):

        tgt_pos = seq_range * (torch.le(seq_range, length_t.view(-1,1))).long()
        dec_output, *_ = self.decoder(s_t_1, tgt_pos, s0, enc_output) # bs,l,d
        #pooled = nn.MaxPool1d(self.length)(output.permute(0,2,1)).squeeze(2) #batch,2h
        pooled = dec_output[:,pos,:]
        # 2*hid+1 +length
        pos_tensor = torch.zeros(self.batch_size,self.length,1).to(self.device)
        pos_tensor[:,pos,:] = 1
        representation = torch.cat([pooled,key_pos[:,pos:pos+1], pos_tensor.squeeze(2)],1)
        
        rep_representation = self.mlp_rep(representation).unsqueeze(2)
        ins_representation = self.mlp_ins(representation).unsqueeze(2)
        del_representation = self.mlp_del(representation).unsqueeze(2)
        hold_score = self.mlp_hold(representation)
        del_score = self.mlp_del(representation)
        ...
        onehot = torch.zeros(K,K).scatter_(1,tag,int(1)).to(self.device)
        # bs,80
        rep_words = torch.cat([rep_f_word, rep_b_word], 1)
        ins_words = torch.cat([ins_f_word, ins_b_word], 1)

        rep_emb = embmodel.encoder(rep_words).detach()
        emb = torch.cat([rep_emb, onehot.unsqueeze(0).repeat(self.batch_size,1,1)],2) #bs,82,382
        emb = self.mlp1(emb)
        rep_score = torch.bmm(emb, rep_representation).squeeze(2) # bs,82,382; bs,382,1-> bs,82,1

        ins_emb = embmodel.encoder(rep_words).detach()
        emb = torch.cat([ins_emb, onehot.unsqueeze(0).repeat(self.batch_size,1,1)],2) #bs,82,382
        emb = self.mlp1(emb)
        ins_score = torch.bmm(emb, rep_representation).squeeze(2) # bs,82,382; bs,382,1-> bs,82,1
        score = torch.cat([rep_score, ins_score, del_score, hold_score],1)
        policy = nn.Softmax(1)(score) # bs,V
