...
def similarity_keyword(s1_list, s2, sta_vec, id2sen, emb_word, option, model = None):
    ...
    for s1 in s1_list:
        ...
        sim = np.mean([x for x,y in zip(list(sim_vec*wei2),list(wei2)) if y>0]+[1])
        #sim = sigma_word(sim)
        sims.append(sim)
    ...
    for s1 in s1_list:
        ...
        generated_word_lists = [id2sen(s1)]
        bleu_score = get_corpus_bleu_scores(actual_word_lists, generated_word_lists)[1]
        bleu_score = 0.5*(1-bleu_score) + 0.5 # 0.5-1
        bleus.append(bleu_score)

    semantics = np.power(np.array(sims),10)
    diff = np.power(np.array(bleus),1)
    res = diff*semantics
    return res
...
def similarity_batch(s1_lists, s2s, sta_vecs, id2sen, emb_word, option, model = None):
    simss= []
...
def similarity_batch1(s1_lists, s2s, sta_vecs, id2sen, emb_word, option, simfun, model = None):
    simss= []
    for s1_list,s2, sta_vec in zip(s1_lists,s2s, sta_vecs):
