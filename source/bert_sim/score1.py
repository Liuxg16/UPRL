...
class Bert_sim(object):
    ...
    def cal_sim(self, cands, refs, idf= True, batch_size=64, nthreads=4):
        if not idf:
            ...
            self.idf_dict = get_idf_dict(refs, self.tokenizer, nthreads=nthreads)

        all_preds = bert_cos_score_idf(self.model, refs, cands, self.tokenizer, self.idf_dict,
                                       device=self.device, 
                                       batch_size=batch_size, all_layers=self.all_layers)
